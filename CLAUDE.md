...
- `skills/ohospatch/references/fixit.d.js` `@version` **must equal** `Fixit.runtimeVersion` in `ohospatch/src/main/cpp/runtime/fixit.js` (currently `1.9.0`). Signatures, constraints, and globals in the declaration must match the runtime.
...
State rules use `state(name, value)` or `state(name, function (originValue) { ... })`; the function receives the original state and `this` is the synchronous current-component Proxy. The old `state(name).replace/transform` chain is unsupported.
