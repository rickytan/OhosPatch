...
- To patch nodes created by an `@Builder` method declared directly on the target Component, use `Fixit.component(componentFullPath).builder(builderMethodName).node(nodeSelector)`. The generated Builder method must remain a callable prototype method. Its per-type occurrence counters reset for each synchronous invocation; normal functions in attribute/event rules bind `this` to the current Component instance.
...
- Component DSL supports API 20 state-management V1 and V2 exported custom components, direct `@Builder` method scopes, parent-provided BuilderParam scopes, preferred `type + occurrence` selection, `type + where` original-attribute selection, create/attribute argument replacement with nested function callbacks, `$r` resource values, and synchronous event replacement. Not supported: `before`/`after`/`around` event composition, non-exported `@Entry` route pages, hierarchy selectors, arbitrary `Resource`/Controller object bridging, and forced refresh of mounted components.
